...
export function ReviewManager() {
  ...
  const loadReviews = async () => {
    ...
    try {
      ...
      setFeedback({
        message: "Неуспешно зареждане на отзивите. Опитайте отново.",
        variant: "error",
      });
    } finally {
      setLoading(false);
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (editingId) {
      ...
      try {
        ...
        setReviews((prev) =>
          prev.map((review) => (review.id === editingId ? updated : review)),
        );
        setFeedback({ message: "Отзивът беше обновен.", variant: "success" });
      } catch (error) {
        console.error("Failed to update review", error);
        setReviews(previousReviews);
        setFeedback({
          message: "Неуспешно запазване на отзива.",
          variant: "error",
        });
        return;
      }
      ...
      try {
        ...
        setReviews((prev) =>
          prev.map((review) =>
            review.id === optimistic.id ? created : review,
          ),
        );
        setFeedback({
          message: "Отзивът беше добавен успешно.",
          variant: "success",
        });
      } catch (error) {
        ...
        setReviews(previousReviews);
        setFeedback({
          message: "Неуспешно добавяне на отзив.",
          variant: "error",
        });
        return;
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setFeedback({ message: "Отзивът беше изтрит.", variant: "success" });
    } catch (error) {
      console.error("Failed to delete review", error);
      setReviews(previousReviews);
      setFeedback({
        message: "Неуспешно изтриване на отзива.",
        variant: "error",
      });
    }
  };
          ...
          <div>
            ...
            <select
              ...
              onChange={(event) =>
                setForm((prev) => ({
                  ...prev,
                  status: event.target.value as ClientReview["status"],
                }))
              }
            >
              ...
              .map((review) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-lg font-semibold text-boho-brown boho-heading">
