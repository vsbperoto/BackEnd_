...
import { getSupabaseEnvValue, supabaseClient } from "../lib/supabaseClient";
import {
  Mail,
  Lock,
  Send,
  ...
  const callEdgeFunctionWithInvoke = async () => {
    try {
      ...
      const { data, error: invokeError } =
        await supabaseClient.functions.invoke("client-gallery-auth", {
      ...
      if (invokeError) {
        ...
        setError(
          `Invoke Error: ${invokeError.message || "Failed to invoke function"}`,
        );
        return;
      }
      ...
      setError(
        `Unexpected Error: ${err.message || "An unexpected error occurred."}`,
      );
    }
  };
    ...
    <div className="boho-card rounded-boho p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-boho-rust font-boho mt-1">
            Test the `client-gallery-auth` Edge Function to debug CORS and
            authentication issues.
          </p>
        </div>
        <div className="flex items-center space-x-2">
          <Globe className="w-5 h-5 text-boho-sage" />
          <span className="text-sm text-boho-rust font-boho">
            localhost:5173 → Supabase
          </span>
        </div>
      </div>
        ...
        <div className="flex space-x-4">
          <label className="flex items-center space-x-2 cursor-pointer">
            <input
              ...
              onChange={(e) =>
                setCallMethod(e.target.value as "invoke" | "fetch")
              }
              className="w-4 h-4 text-boho-sage border-boho-brown focus:ring-boho-sage"
            />
            <span className="text-sm text-boho-brown font-boho">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-boho p-4">
        <div className="flex items-start space-x-2">
          ...
          <div>
            <p className="text-sm font-medium text-blue-800 font-boho mb-2">
              Debugging Tips:
            </p>
            <ul className="text-xs text-blue-700 space-y-1 font-boho">
              <li>
                • Open Browser DevTools (F12) → Network tab before calling
              </li>
              <li>
                • Look for OPTIONS preflight request (should return 200 OK)
              </li>
              <li>
                • Check CORS headers in response: Access-Control-Allow-Origin
              </li>
              <li>
                • Verify function URL in network request matches your Supabase
                project
              </li>
        ...
        <div className="bg-green-50 border border-green-200 rounded-boho p-4">
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-green-800 font-boho mb-2">
                ✅ Function Response:
              </p>
      ...
      {error && (
        <div className="bg-red-50 border border-red-200 rounded-boho p-4">
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-red-800 font-boho mb-2">
                ❌ Error Details:
              </p>
              ...
              <div className="mt-3 text-xs text-red-600 font-boho">
                ...
                <ul className="space-y-1 ml-4">
                  <li>
                    • CORS Error: Check Edge Function CORS headers and OPTIONS
                    handling
                  </li>
                  <li>
                    • 401/403: Verify Supabase API keys and function permissions
                  </li>
                  <li>
                    • 404: Ensure Edge Function is deployed and URL is correct
                  </li>
                  <li>
                    • Network Error: Check internet connection and Supabase
                    status
                  </li>
                </ul>
              </div>
      ...
      <div className="bg-boho-sage bg-opacity-10 rounded-boho p-4 border border-boho-sage border-opacity-30">
        <p className="text-sm font-medium text-boho-brown font-boho mb-2">
          Function URL:
        </p>
        <code className="text-xs text-boho-brown bg-boho-cream bg-opacity-50 px-2 py-1 rounded border font-mono break-all">
          {getSupabaseEnvValue("SUPABASE_URL") || "SUPABASE_URL not set"}
          /functions/v1/client-galleries
        </code>
      </div>
    </div>
  );
