...
async function fetchPricingPackageWithRelations(id) {
  ...
  const [{ data: featureData, error: featureError }, { data: tierData, error: tierError }] = await Promise.all([
    supabase
...
adminRouter.get('/galleries', async (req, res) => {
  try {
    const { data, error } = await supabase
      .from('galleries')
      .select('id, title, subtitle, event_date, cover_image, images, created_at')
      .order('created_at', { ascending: false });
...
adminRouter.post('/contacts/:id/archive', async (req, res) => {
  try {
    ...
    const { error } = await supabase
      ...
      .upsert({ contact_id: id, archived_at: new Date().toISOString() }, { onConflict: 'contact_id' });
    if (error) return mapSupabaseError(res, error);
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: String(err) });
...
adminRouter.post('/client_images/reorder', async (req, res) => {
  try {
    ...
    for (const item of order) {
      ...
      const result = await supabase.from('client_images').update({ order_index }).eq('id', id);
      if (result.error) {
...
adminRouter.post('/blog/posts', async (req, res) => {
  try {
    ...
    const insertPayload = {
      ...
      updated_at: now,
      published_at:
        payload.status === 'published' && !payload.published_at ? now : payload.published_at || null
    };
    ...
    if (Array.isArray(tags) && tags.length > 0) {
      const rows = tags.map((tagId) => ({ post_id: created.id, tag_id: tagId }));
      const { error: tagError } = await supabase.from('blog_post_tags').upsert(rows, {
        onConflict: 'post_id,tag_id'
      });
      if (tagError) return mapSupabaseError(res, tagError);
    }

    const { post, error: fetchError } = await fetchBlogPostWithTags(created.id);
  ...
  try {
    ...
    if (Array.isArray(tags)) {
      ...
      if (tags.length > 0) {
        ...
        const { error: addError } = await supabase.from('blog_post_tags').insert(rows);
...
adminRouter.post('/pricing/packages', async (req, res) => {
  try {
    ...
    if (Array.isArray(tiers) && tiers.length > 0) {
      ...
      const { error: tierError } = await supabase.from('pricing_package_tiers').insert(tierRows);
      if (tierError) return mapSupabaseError(res, tierError);
    }

    const { pkg, error: fetchError } = await fetchPricingPackageWithRelations(created.id);
    if (fetchError) return mapSupabaseError(res, fetchError);
    res.json(pkg);
  } catch (err) {
    res.status(500).json({ error: String(err) });
...
adminRouter.patch('/pricing/packages/:id', async (req, res) => {
  try {
    ...
    const { pkg, error: fetchError } = await fetchPricingPackageWithRelations(id);
    if (fetchError) return mapSupabaseError(res, fetchError);
    res.json(pkg);
  } catch (err) {
    res.status(500).json({ error: String(err) });
